...
function SaveData(command) {
    try {
        ...
        $.ajax({
            type: "POST",
            url: "http://www.tadreeb.ws/code/customer.ashx",
            ...
            success: function (par) {
                ...
                for (i = 1; i < arr.length; i++) {
                    var d=arr[i].split("++");
                    $('<li data-icon="false">').append('<h2> customer ID : ' + d[0] + '</h2><h2> Customer Name : ' + d[1] + '</h2><h2> Phone : ' + d[2] + '</h2> <div data-role="controlgroup" data-type="horizontal"><a href="javascript:display(\'' + d[0] + '\',\'' + d[1] + '\',\'' + d[2] + '\',\'' + d[3] + '\',\'' + d[4] + '\')" class="ui-btn ui-btn-inline ui-shadow ui-icon-edit  ui-btn-icon-notext"></a> <a href="javascript:Delete(\'' + d[0] + '\')" class="ui-btn ui-btn-inline ui-shadow ui-icon-delete  ui-btn-icon-notext"></a></div>').appendTo('#ldata');
                }
